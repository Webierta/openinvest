...
class _FundDetailsPageState extends State<FundDetailsPage> {
  ...
  void _showPriceDeleteError(BuildContext context, FundData fund) {
    if (!mounted) return;
    ScaffoldMessenger.of(context).showSnackBar(
      SnackBar(
        ...
        action: SnackBarAction(
          ...
          onPressed: () => ExportService.exportFund(context, fund),
        ),
      ),
    );
  }
  ...
  Widget _buildSummaryCard(
    ...
    if (fund.history.length > 1) {
      ...
      final oldestVal = fund.history.first.price;
      oldestDate = fund.history.first.date;
      if (oldestVal != 0) {
        totalDiff = fund.lastValue - oldestVal;
  ...
  Widget _buildBalanceSection(
      ...
      if (daysDiff > 0) {
        ...
        if (firstOpPricePoint != null && firstOpPricePoint.price > 0) {
          final double twrTotal =
              (fund.lastValue / firstOpPricePoint.price) - 1;
        ...
        List<Map<String, Object>> flows = fund.operations
            .map(
              (op) => <String, Object>{
    ...
    return Column(
      ...
      children: [
        ...
        _buildBalanceRow(
          'Precio medio compra',
          '${priceFormat.format(avgPurchasePrice)} ${fund.currency}',
        ),
        _buildBalanceRow(
          'Valor actual',
        ...
        Row(
          children: [
            ...
            const SizedBox(width: 12),
            Expanded(
              child: _buildSimpleStat('MWR Anualizado', mwrAnnualizedStr),
            ),
          ],
        ),
        const SizedBox(height: 12),
        Row(
          children: [
            ...
            Expanded(
              child: _buildSimpleStat(
                'Break-even',
                priceFormat.format(avgPurchasePrice),
              ),
            ),
          ],
        ),
        ...
        Row(
          ...
          children: [
            ...
            Column(
              ...
              children: [
                Text(
                  '${profitAbs > 0 ? '+' : ''}${smartFormat.format(profitAbs)} ${fund.currency}',
                  style: TextStyle(
                    color: profitColor,
                    fontWeight: FontWeight.bold,
                    fontSize: 16,
                ...
                Text(
                  '${profitRel > 0 ? '+' : ''}${percentFormat.format(profitRel)}%',
                  style: TextStyle(
                    color: profitColor,
                    fontWeight: FontWeight.bold,
                  ),
    ...
    if (_selectedChartRange != 'ALL') {
      ...
      switch (_selectedChartRange) {
        case '1M':
          ...
          break;
        case '6M':
          cutoff = now.subtract(const Duration(days: 182));
          break; // ~6 meses
        case 'YTD':
        ...
        Row(
          ...
          children: [
            Padding(
              ...
              child: Text(
                'Evolución (${filteredHistory.length} pts)',
                style: const TextStyle(
                  color: Colors.white70,
                  fontWeight: FontWeight.bold,
                ),
            ...
            LineChartData(
                  ...
                  sideTitles: SideTitles(
                    ...
                    getTitlesWidget: (v, m) {
                      ...
                      return Padding(
                        ...
                        child: Text(
                          DateFormat('dd/MM').format(filteredHistory[i].date),
                          style: const TextStyle(
                            fontSize: 9,
                            color: Colors.white38,
                          ),
              ...
              lineTouchData: LineTouchData(
                touchTooltipData: LineTouchTooltipData(
                  getTooltipColor: (s) => const Color(0xFF1E293B),
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          '1Y',
          'ALL',
        ].map((r) => _buildRangeButton(r)).toList(),
      ),
    );
  }
  ...
  Widget _buildTableSection(
    ...
    for (int i = 0; i < fund.history.length; i++) {
      ...
      double? absVar, variation;
      if (i > 0) {
        final prev = fund.history[i - 1].price;
        absVar = point.price - prev;
        if (prev != 0) variation = (absVar / prev) * 100;
      }
      tableData.add({
        'index': i + 1,
        'date': point.date,
    ...
    tableData.removeWhere(
      (row) => _pendingDeletedPricePoints.contains(
        '${fund.isin}|${(row['date'] as DateTime).toIso8601String()}',
      ),
    );
    tableData.sort((a, b) {
      int cmp;
      switch (_sortColumnIndex) {
        case 0:
          cmp = (a['index'] as int).compareTo(b['index'] as int);
          break;
        case 1:
          cmp = (a['date'] as DateTime).compareTo(b['date'] as DateTime);
          break;
        case 2:
          cmp = (a['price'] as double).compareTo(b['price'] as double);
          break;
        case 3:
          cmp = (a['absVar'] as double? ?? 0).compareTo(
            b['absVar'] as double? ?? 0,
          );
          break;
        case 4:
          cmp = (a['variation'] as double? ?? 0).compareTo(
    ...
    return Column(
      children: [
        Container(
          ...
          child: Row(
            children: [
              Expanded(flex: 8, child: _buildSortableHeader('No.', 0)),
              Expanded(flex: 20, child: _buildSortableHeader('Fecha', 1)),
              Expanded(flex: 18, child: _buildSortableHeader('Precio', 2)),
              Expanded(flex: 15, child: _buildSortableHeader('Diff.', 3)),
              Expanded(flex: 12, child: _buildSortableHeader('Var.', 4)),
            ],
          ),
        ),
        Container(
          ...
          child: ListView.separated(
            ...
            itemBuilder: (context, index) {
              ...
              final absVar = row['absVar'] as double?,
                  variation = row['variation'] as double?;
              Color? vColor;
              if (absVar != null) {
                vColor = absVar > 0
                    ? Colors.greenAccent[400]
                    : (absVar < 0 ? Colors.redAccent[200] : null);
              }
              return Dismissible(
                key: ValueKey(row['date'].toString()),
                ...
                confirmDismiss: (direction) async {
                  return await showDialog(
                    ...
                    builder: (context) => AlertDialog(
                      ...
                      content: Text(
                        '¿Deseas eliminar el registro del día ${DateFormat("dd/MM/yyyy").format(row["date"])}?',
                      ),
                      actions: [
                        TextButton(
                ...
                onDismissed: (direction) async {
                  ...
                  final deletedPoint = PricePoint(date, row['price'] as double);
                  final pointKey = '${fund.isin}|${date.toIso8601String()}';
                  _pendingDeletedPricePoints.add(pointKey);
                  tableData.remove(row);
                  setState(() {});
                  try {
                    final provider = context.read<FundProvider>();
                    await provider.deletePricePoint(fund.isin, date);
                    _pendingDeletedPricePoints.remove(pointKey);
                      ...
                      SnackBar(
                        ...
                        action: SnackBarAction(
                          ...
                          onPressed: () async {
                            try {
                              final provider = context.read<FundProvider>();
                              await provider.restorePricePoint(
                                fund.isin,
                                deletedPoint,
                              );
                              if (!context.mounted) return;
                              ...
                              if (context.mounted) {
                                _showPriceDeleteError(context, fund);
                              }
                            }
                          },
                        ),
                    ...
                    children: [
                      ...
                      Expanded(
                        ...
                        child: Padding(
                          ...
                          child: Text(
                            absVar != null
                                ? '${absVar > 0 ? "+" : ""}${priceFormat.format(absVar)}'
                                : '-',
                            style: TextStyle(
                              color: vColor,
                              fontWeight: FontWeight.bold,
                      ...
                      Expanded(
                        ...
                        child: Padding(
                          ...
                          child: Text(
                            variation != null
                                ? '${variation > 0 ? "+" : ""}${percentFormat.format(variation)}%'
                                : '-',
                            style: TextStyle(
                              color: vColor,
                              fontWeight: FontWeight.bold,
  ...
  Widget _buildStatsGrid(
    ...
    if (fund.history.isEmpty) return const SizedBox.shrink();
    double sum = 0;
    PricePoint maxP = fund.history.first, minP = fund.history.first;
    List<double> returns = [];
    for (int i = 0; i < fund.history.length; i++) {
      final p = fund.history[i];
      sum += p.price;
      if (p.price > maxP.price) maxP = p;
      if (p.price < minP.price) minP = p;
      if (i > 0) {
        final prev = fund.history[i - 1].price;
        if (prev != 0) returns.add((p.price - prev) / prev);
      }
    }
    final meanPrice = sum / fund.history.length;
    double volAnual = 0;
    if (returns.isNotEmpty) {
      double meanRet = returns.reduce((a, b) => a + b) / returns.length;
      double varSum = returns
          .map((r) => pow(r - meanRet, 2).toDouble())
          .reduce((a, b) => a + b);
      volAnual = sqrt(varSum / returns.length) * sqrt(252) * 100;
    }
      ...
      children: [
        Padding(
          ...
          child: ElevatedButton.icon(
            onPressed: () => _showOperationDialog(context, provider, fund),
          ...
          child: fund.operations.isEmpty
              ...
              : ListView.separated(
                  ...
                  separatorBuilder: (context, index) =>
                      const Divider(height: 1, color: Colors.white10),
                  itemBuilder: (context, index) {
                    final op = fund.operations[index];
                    ...
                    return ListTile(
                      ...
                      trailing: Column(
                        ...
                        children: [
                          Text(
                            '${isBuy ? '' : '-'}${smartFormat.format(op.amount)} ${fund.currency}',
                            style: TextStyle(
                              fontWeight: FontWeight.bold,
                              color: isBuy
                                  ? Colors.greenAccent[400]
                          ...
                          Text(
                            '${smartFormat.format(op.units)} part. @ ${smartFormat.format(op.price)}',
                            style: const TextStyle(
                              color: Colors.white38,
                              fontSize: 11,
                            ),
  ...
  void _showOperationDialog(
    ...
    if (operation != null) {
      unitsController.text = operation.units.toString().replaceAll('.', ',');
      priceController.text = operation.price.toString().replaceAll('.', ',');
      amountController.text = operation.amount.toString().replaceAll('.', ',');
    } else {
      priceController.text = fund.lastValue.toString().replaceAll('.', ',');
    }
    void updateAmount() {
              ...
              children: [
                SegmentedButton<OperationType>(
                  ...
                  onSelectionChanged: (newS) =>
                      setState(() => selectedType = newS.first),
                ),
                const SizedBox(height: 16),
                ListTile(
                  ...
                  onTap: () async {
                    ...
                    if (p != null) {
                      setState(() {
                        selectedDate = p;
                        if (operation == null) {
                          final hp = fund.history
                              .cast<PricePoint?>()
                              .firstWhere(
                                (x) =>
                                    x!.date.year == p.year &&
                                    x.date.month == p.month &&
                                    x.date.day == p.day,
                                orElse: () => null,
                              );
                          if (hp != null) {
                            priceController.text = hp.price
                                .toString()
                                .replaceAll('.', ',');
                            updateAmount();
                          }
